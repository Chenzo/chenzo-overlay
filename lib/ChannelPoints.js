...
const validateToken = async (twitchAccessToken) => {
   ...
   /* try {
    ...
    userId = process.env.NEXT_PUBLIC_USERID //r.user_id //process.env.NEXT_PUBLIC_USERID
    headers = {
...
const getCustomRewards = async () => {
    try {
        let { body } = await fetch(`https://api.twitch.tv/helix/channel_points/custom_rewards?broadcaster_id=${userId}`, {method: 'GET', headers: headers })
